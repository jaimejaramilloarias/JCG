export function readU32(dv, o){ return dv.getUint32(o, false); }
export function readU16(dv, o){ return dv.getUint16(o, false); }

export function readVarLen(u8, i) {
  let value = 0;
  for (let k=0; k<4; k++){
    const b = u8[i++];
...
export function str4(u8, o) {
  return String.fromCharCode(u8[o],u8[o+1],u8[o+2],u8[o+3]);
}

export function parseTrack(trackU8) {
  let i = 0;
  let absTick = 0;
...
export function buildMidiFile(ppq, channelEventsAbsTicks) {
  const trackBytes = [];
...
export function enforceMaxNoteLength(events, tickPerEighth, maxEighths=3) {
